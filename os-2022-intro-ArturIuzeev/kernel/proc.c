...
int dump(void) {
  ...
  printf("%d %d %d %d %d %d %d %d %d %d\n", p->trapframe->s2, p->trapframe->s3,
         p->trapframe->s4, p->trapframe->s5, p->trapframe->s6, p->trapframe->s7,
         p->trapframe->s8, p->trapframe->s9, p->trapframe->s10,
         p->trapframe->s11);
  return 0;
}
...
int dump2(int pid, int register_num, uint64 *return_value) {
  ...
  struct proc *proc1;
  int count = 0;
  for (proc1 = proc; proc1 < &proc[NPROC]; proc1++) {
    if (proc1->pid == pid) {
      count = 1;
      break;
    }
  ...
  uint64 *process = &proc1->trapframe->s2;
  ...
  process += register_num - 2;
  int res =
      copyout(p->pagetable, *return_value, (char *)process, sizeof(*process));
