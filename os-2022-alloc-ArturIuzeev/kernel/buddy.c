...
int bit_is_set_mod(char *array, int index) {
  index >>= 1;
  char b = array[index / 8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
void bd_print_vector(char *vector, int len) {
  ...
  lb = 0;
  for (int b = 0; b < len; b++) {
    if (last == bit_is_set_mod(vector, b)) continue;
    if (last == 1) printf(" [%d, %d)", lb, b);
    lb = b;
    last = bit_is_set_mod(vector, b);
  }
  if (lb == 0 || last == 1) {
    printf(" [%d, %d)", lb, len);
  }
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (bit_is_set_mod(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
int check_range(void *free_start, void *free_end, void *x) {
  return x >= free_start && x < free_end;
}

// If a block is marked as allocated and the buddy is free, put the
// buddy on the free list at size k.
int bd_initfree_pair(int k, int bi, void *free_start, void *free_end) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_is_set_mod(bd_sizes[k].alloc, bi)) {
    free = BLK_SIZE(k);
    if (check_range(free_start, free_end, addr(k, buddy))) {
